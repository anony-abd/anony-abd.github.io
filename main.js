...
function detectODEVariables(userInput) {
    ...
    if (primeMatch) {
        ...
        let exclude = ['e', 'pi', 'i', 'sin', 'cos', 'tan', 'cot', 'sec', 'csc', 'sinh', 'cosh', 'tanh', 'ln', 'log', 'exp', 'diff', 'integrate', 'laplace', 'ilaplace'];
        let m;
...
function kaTeXDisplay(type, rows = 2, cols = 2) {
    ...
    switch (type) {
        ...
        case 'laplace': insertText = '\\mathcal{L}\\left\\{{}\\right\\}(s)'; break;
        case 'ilt': insertText = '\\mathcal{L}^{-1}\\left\\{{}\\right\\}(t)'; break;
...
if (typeof document !== 'undefined') {
    document.addEventListener('DOMContentLoaded', function () {
        const renders = {
            ...
            'btn-math-lte': '\\le',
            'btn-math-gte': '\\ge',
            'btn-math-laplace': '\\text{laplace}',
            'btn-math-ilt': '\\text{ilt}',
...
function setMode(mode) {
    ...
    const useLatex = (mode === 'math' || isToolsActive);
        ...
        if (opMatch) {
            ...
            if (idx >= 0) {
                ...
                if (operandStart !== -1) {
                    ...
                    if (trigFuncs.includes(operand.toLowerCase())) {
                        i += opLen;
                        continue;
                    }
                    ...
                    if (exponentStr === undefined) {
                        replacement = `transpose(${operand})`;
                    } else {
                        let powerVal = parseInt(exponentStr, 10);
                        if (powerVal === 1) {
                            replacement = operand;
                        } else if (powerVal === -1) {
                            replacement = `invert(${operand})`;
                        } else if (powerVal > 1) {
                            replacement = operand;
                            for (let p = 1; p < powerVal; p++) {
                                replacement = `multiply(${replacement}, ${operand})`;
                            }
                        } else if (powerVal < -1) {
                            let invOperand = `invert(${operand})`;
                    ...
                    str = str.substring(0, operandStart) + replacement + str.substring(i + opLen);
                    i = 0;
                    continue;
                }
            }
    ...
    try {
        if (processedInput.includes('=')) {
            ...
            if (_core && expr.symbol instanceof _core.Matrix) {
                ...
                let simplified = processedInput.includes('partfrac') ? expr : expr.simplify();
